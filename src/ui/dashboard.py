...
app.layout = html.Div([
    ...
    # Header
    html.Div([
        html.H1("⚡ Aether-Flow", style={"margin": "0", "color": "#fff", "fontSize": "28px"}),
    ...
    html.Div([
        ...
        html.Div([
            ...
            # Goal input
            html.Div([
                html.H3("Run Pipeline", style={"color": "#e2e8f0", "marginBottom": "12px"}),
                dcc.Textarea(
                    id="goal-input",
                ...
                html.Div(id="run-status", style={"marginTop": "12px", "fontSize": "13px"})
            ], style={
            ...
            html.Div([
                html.H3("Pipeline Status", style={"color": "#e2e8f0", "marginBottom": "12px"}),
            ...
            # Example goals
            html.Div([
                html.H3("Example Goals", style={"color": "#e2e8f0", "marginBottom": "12px"}),
                html.Div([
                    html.Button(
    ...
    ], style={"display": "flex", "padding": "24px", "background": "#171923", "minHeight": "calc(100vh - 80px)"}),

], style={"background": "#171923", "fontFamily": "system-ui, sans-serif", "minHeight": "100vh"})
...
def fetch_stats():
    try:
        ...
        return {"overall": {}, "repair_patterns": [], "recent_runs": []}


def phase_badge(phase: str, current: str) -> html.Span:
    phases = ["discovery", "planning", "executing", "auditing", "complete"]
    labels = {"discovery": "① Discovery", "planning": "② Planning",
              "executing": "③ Execution", "auditing": "④ Guardian", "complete": "✅ Complete"}
    ...
    if phase == current and phase != "complete":
        bg, color = "#4299e1", "#fff"
    elif phase_idx < current_idx or current == "complete":
        bg, color = "#48bb78", "#fff"
    else:
...
def start_pipeline(n_clicks, goal):
    if not goal or not goal.strip():
        return html.Span("⚠️ Please enter a goal.", style={"color": "#f6ad55"}), None, True
    ...
    try:
        ...
        return (
            html.Span(f"✓ Pipeline started: {job_id}", style={"color": "#68d391"}),
            job_id,
            False
        )
    except Exception as e:
        return html.Span(f"✗ Error: {e}", style={"color": "#fc8181"}), None, True
...
def poll_status(n, job_id):
    if not job_id:
        return html.P("No active pipeline.", style={"color": "#718096"}), True
    ...
    try:
        ...
        phases = ["discovery", "planning", "executing", "auditing", "complete"]
        badges = html.Div([phase_badge(p, phase) for p in phases])

        details = html.Div([
            html.P(f"Job: {job_id}", style={"color": "#718096", "fontSize": "12px", "margin": "8px 0 2px"}),
            html.P(f"Status: {status} | Phase: {phase}",
                   style={"color": "#a0aec0", "fontSize": "12px", "margin": "2px 0"}),
        ])

        result_section = html.Div()
        if status == "complete":
            try:
                rr = requests.get(f"{API_BASE}/pipeline/{job_id}/result", timeout=5)
                ...
                result_section = html.Div([
                    ...
                    html.P(
                        ...
                        style={"color": "#68d391" if deploy else "#fc8181", "fontSize": "13px"}
                    )
                ])
            except:
                pass
        ...
        return html.P(f"Poll error: {e}", style={"color": "#fc8181", "fontSize": "12px"}), False
...
def refresh_telemetry(n):
    ...
    cards = html.Div([
        html.Div([
            html.P(label, style={"color": "#a0aec0", "fontSize": "12px", "margin": "0"}),
            html.H2(str(value), style={"color": color, "margin": "4px 0 0", "fontSize": "28px"})
        ], style={
    ...
    if recent:
        table = html.Table([
            html.Thead(html.Tr([
                html.Th(h, style={"color": "#a0aec0", "fontSize": "12px",
            ...
            html.Tbody([
                html.Tr([
                    html.Td(r["run_uuid"][:8], style={"padding": "8px", "fontSize": "12px", "color": "#a0aec0"}),
                    html.Td(r["goal"][:40] + "...", style={"padding": "8px", "fontSize": "12px", "color": "#e2e8f0"}),
                    html.Td(f"{r['steps_succeeded']}/{r['total_steps']}",
                            style={"padding": "8px", "fontSize": "12px", "color": "#68d391"}),
                    html.Td(str(r["total_repairs"]),
                            style={"padding": "8px", "fontSize": "12px",
                                   "color": "#ed8936" if r["total_repairs"] > 0 else "#68d391"}),
                    html.Td(f"{r['execution_time']:.0f}s",
                            style={"padding": "8px", "fontSize": "12px", "color": "#a0aec0"}),
                    html.Td(
